...
function initGlobe(canvasId) {
  ...
  // Cap DPR at 1.5 (instead of 2) — 30% less pixels to draw on retina screens
  ...
  function draw() {
    ...
    // ── Latitude lines (10° resolution — half the points = faster) ──
  ...
  let lastFrameTime = 0;
  const FRAME_INTERVAL = 1000 / 30; // cap at 30fps — half the CPU work

  function animate(now) {
    if (!animating) return;
    if (!now) now = performance.now();
    if (now - lastFrameTime >= FRAME_INTERVAL) {
      rotation += 0.006; // double rotation to compensate for half FPS
      draw();
      lastFrameTime = now;
    }
  ...
  // ── Pause briefly during user interactions (scroll, click) to free CPU ──
  let pauseTimer = null;
  ...
  // Use passive listeners so they don't block scroll
