...
function copyIBAN() {
  ...
  const ICON_COPY  = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>';
  ...
  navigator.clipboard.writeText(iban).then(() => {
    if (btn) {
      ...
      btn.innerHTML = `${ICON_CHECK} <span>تم النسخ</span>`;
      setTimeout(() => {
        btn.classList.remove('copied');
        btn.innerHTML = `${ICON_COPY} <span>نسخ رقم الحساب</span>`;
      }, 2000);
    }
...
// ── Amount Stepper (+ / − buttons, snap to 50) ───────────────
function setupAmountStepper() {
  const input  = document.getElementById('amount');
  const minus  = document.getElementById('amountMinus');
  const plus   = document.getElementById('amountPlus');
  ...
  function setValue(v) {
    // snap to nearest 50, enforce minimum
    v = Math.max(MIN, Math.round(v / STEP) * STEP);
    input.value = v;
    // Trigger 'input' event so the share calculator updates
  ...
  // When the user types directly, snap on blur
  input.addEventListener('blur', () => {
    const v = getValue();
    if (v > 0 && v % STEP !== 0) setValue(v);
  });

  // Keep button states in sync as the user types
